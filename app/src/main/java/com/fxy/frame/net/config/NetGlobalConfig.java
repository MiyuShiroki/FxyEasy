package com.fxy.frame.net.config;
...
import com.fxy.frame.net.interceptor.GzipRequestInterceptor;
import com.fxy.frame.net.interceptor.OfflineCacheInterceptor;
import com.fxy.frame.net.interceptor.OnlineCacheInterceptor;
...
public class NetGlobalConfig {
     ...
     * @return
     */
    public NetGlobalConfig callFactory(Call.Factory factory) {
    ...
    public NetGlobalConfig proxy(Proxy proxy) {
        H.getOkHttpBuilder().proxy(checkNotNull(proxy, "proxy == null"));
        return this;
    }
    ...
    public NetGlobalConfig connectTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            H.getOkHttpBuilder().connectTimeout(timeout, unit);
        } else {
            H.getOkHttpBuilder().connectTimeout(ViseConfig.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        }
        return this;
    }
    ...
    public NetGlobalConfig writeTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            H.getOkHttpBuilder().writeTimeout(timeout, unit);
        } else {
            H.getOkHttpBuilder().writeTimeout(ViseConfig.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        }
        return this;
    }
    ...
    public NetGlobalConfig readTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            H.getOkHttpBuilder().readTimeout(timeout, unit);
        } else {
            H.getOkHttpBuilder().readTimeout(ViseConfig.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        }
        return this;
    }
    ...
    public NetGlobalConfig interceptor(Interceptor interceptor) {
        H.getOkHttpBuilder().addInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public NetGlobalConfig networkInterceptor(Interceptor interceptor) {
        H.getOkHttpBuilder().addNetworkInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public NetGlobalConfig cacheOffline(Cache httpCache) {
        networkInterceptor(new OfflineCacheInterceptor(H.getContext()));
        interceptor(new OfflineCacheInterceptor(H.getContext()));
        this.httpCache = httpCache;
        return this;
    }
    ...
    public NetGlobalConfig cacheOffline(Cache httpCache, final int cacheControlValue) {
        networkInterceptor(new OfflineCacheInterceptor(H.getContext(), cacheControlValue));
        interceptor(new OfflineCacheInterceptor(H.getContext(), cacheControlValue));
        this.httpCache = httpCache;
        return this;
    }
